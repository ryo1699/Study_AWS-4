locals {
  ...
  deploy_bucket_name = "${local.name}-deploy-${var.bucket_name_suffix}"
  app_home           = "/opt/study-aws-4-task2"
  app_service_name   = "study-aws-4-task2-api"
...
resource "aws_iam_role_policy" "app_deploy_artifact_read" {
  name = "${local.name}-read-deploy-artifacts"
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Action = ["s3:GetObject"]
      Resource = [
        "${aws_s3_bucket.deploy_artifacts.arn}/releases/*"
      ]
    }]
  })
}
