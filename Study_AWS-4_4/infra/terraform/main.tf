locals {
  ...
  app_home         = "/opt/study-aws-4-task4"
  app_service_name = "study-aws-4-task4-api"
  app_database_url = "postgresql+psycopg://${var.db_username}:${var.db_password}@${aws_db_instance.postgres.address}:5432/tasks"
}
...
resource "aws_security_group" "app" {
  ...
  description = "Allow ALB to reach FastAPI on EC2."
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "rds" {
  ...
  description = "Allow app EC2 and bastion to reach PostgreSQL."
  vpc_id      = aws_vpc.main.id
